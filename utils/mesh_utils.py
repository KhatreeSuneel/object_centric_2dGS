...
class GaussianExtractor(object):
    ...
    def export_image(self, path):
        ...
        # saving with original image_name
        for viewpoint_cam in tqdm(self.viewpoint_stack, desc="export images"):
            original_name = viewpoint_cam.image_name
            gt = viewpoint_cam.original_image[0:3, :, :]
            save_img_u8(gt.permute(1,2,0).cpu().numpy(), os.path.join(gts_path, f"{original_name}.png"))
            save_img_u8(self.rgbmaps[self.viewpoint_stack.index(viewpoint_cam)].permute(1,2,0).cpu().numpy(), os.path.join(render_path, f"{original_name}.png"))
            save_img_f32(self.depthmaps[self.viewpoint_stack.index(viewpoint_cam)][0].cpu().numpy(), os.path.join(vis_path, f"depth_{original_name}.tiff"))
            # save_img_u8(self.normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'normal_{0:05d}'.format(idx) + ".png"))
            # save_img_u8(self.depth_normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'depth_normal_{0:05d}'.format(idx) + ".png"))
